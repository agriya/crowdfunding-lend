...
class LendChartsController extends ChartsController
{
    public $name = 'LendCharts';
    ...
    public function chart_projects() 
    {
        ...
        if ($this->Auth->user('role_id') == ConstUserTypes::Admin) {
            ...
            if (!empty($city_filter_id)) {
                $project_cities = $this->Lend->User->UserProfile->City->find('first', array(
                    'conditions' => array(
                ...
                foreach($project_cities['Lend'] as $project_city) {
                    $city_project_id[] = $project_city['id'];
                }
                $conditions['Lend.id'] = $city_project_id;
            }
        }
        ...
        $chart_projects_data = $this->_setLineData($select_var, $project_model_datas, 'Lend.Lend', 'Lend', $common_conditions);
        ...
        $project_fund_model_datas['Lent'] = array(
            'display' => __l('Lent') ,
            'conditions' => array(
                'ProjectFund.project_type_id' => ConstProjectTypes::Lend,
