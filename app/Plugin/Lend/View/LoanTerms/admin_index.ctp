...
<div class="main-admn-usr-lst js-response">
	<div class="bg-primary row">		
		<ul class="list-inline sec-1 navbar-btn">
			...
			<li>
				<?php echo $this->Html->link('<span class="img-circle img-thumbnail bg-sucess img-wdt center-block text-center opn-i-usr">'.$this->Html->cInt($pending + $approved,false).'</span><span>' .__l('All'). '</span>', array('controller'=>'loan_terms','action'=>'index'), array('class' => 'text-center','escape' => false));?>
			</li>
		</ul>		
	</div>
	<div class="clearfix">		
		...
		<?php echo $this->Form->create('LoanTerm', array('action' => 'update','class'=>'clearfix js-shift-click js-no-pjax')); ?>
			...
			<table class="table table-striped">
				<thead>
					<tr>      
						...
						<th class="text-center"><?php echo $this->Paginator->sort('months', __l('Months'));?></th>
					</tr>
				</thead>
				<tbody class="h6">
					<?php
					...
					foreach ($loanTerms as $loanTerm):
							 if($loanTerm['LoanTerm']['is_approved'] == '1')  :
					...
					<tr <?php echo $disabled; ?>>
						<td class="text-center">
							<?php
							echo $this->Form->input('LoanTerm.'.$loanTerm['LoanTerm']['id'].'.id',array('type' => 'checkbox', 'id' => "admin_checkbox_".$loanTerm['LoanTerm']['id'],'label' => '' , 'class' => $status_class.' js-checkbox-list'));?>
						</td>
						<td class="text-center">
							<div class="dropdown">
								...
								<ul class="dropdown-menu">
									<li>
									<?php echo $this->Html->link('<i class="fa fa-pencil-square-o fa-fw"></i>'.__l('Edit'), array( 'action'=>'edit', $loanTerm['LoanTerm']['id']), array('class' => '','escape'=>false, 'title' => __l('Edit')));?>
									</li>
									<li>
									<?php echo $this->Html->link('<i class="fa fa-times fa-fw"></i>'.__l('Delete'), Router::url(array('action'=>'delete',$loanTerm['LoanTerm']['id']),true).'?r='.$this->request->url, array('class' => 'js-confirm ', 'escape'=>false,'title' => __l('Delete')));?>
									</li>
									<?php echo $this->Layout->adminRowActions($loanTerm['LoanTerm']['id']);  ?>
								</ul>
							</div>
						</td>
						<td class="text-left"><?php echo $this->Html->cText($loanTerm['LoanTerm']['name']);?></td>
						<td class="text-center"><?php echo $this->Html->cInt($loanTerm['LoanTerm']['months']);?></td>
					</tr>
					<?php
					endforeach;
					else:
					?>
					<tr>
						<td colspan="7" class="text-center text-danger"><i class="fa fa-exclamation-triangle fa-fw"></i><?php echo sprintf(__l('No %s available'), __l('loan terms'));?></td>
					</tr>
					<?php
					endif;
					?>
