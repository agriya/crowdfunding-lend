...
<div class="main-admn-usr-lst js-response">
	<div class="bg-primary row">		
		<ul class="list-inline sec-1 navbar-btn">
			...
			<li>
				<?php echo $this->Html->link('<span class="img-circle img-thumbnail bg-sucess img-wdt center-block text-center opn-i-usr">'.$this->Html->cInt($pending + $approved,false).'</span><span>' .__l('All'). '</span>', array('controller'=>'repayment_schedules','action'=>'index'), array('class' => 'text-center','escape' => false));?>
			</li>
		</ul>		
	</div>
	<div class="clearfix">		
		...
		<?php echo $this->Form->create('RepaymentSchedule', array('action' => 'update','class'=>'clearfix js-shift-click js-no-pjax')); ?>
			...
			<table class="table table-striped">
				<thead class="h5">
					<tr>      
						...
						<th class="text-center"><?php echo $this->Paginator->sort('day', __l('Day'));?></th>
						<th class="text-center"><?php echo $this->Paginator->sort('is_particular_day_of_month', __l('Particular Day of Month?'));?></th>
					</tr>
				</thead>
				<tbody class="h5">
					<?php
					if (!empty($repaymentSchedules)):
					foreach ($repaymentSchedules as $repaymentSchedule):
					if($repaymentSchedule['RepaymentSchedule']['is_approved'] == '1')  :
					...
					<tr <?php echo $disabled; ?>>
						<td class="text-center">
							<?php
							echo $this->Form->input('RepaymentSchedule.'.$repaymentSchedule['RepaymentSchedule']['id'].'.id',array('type' => 'checkbox', 'id' => "admin_checkbox_".$repaymentSchedule['RepaymentSchedule']['id'],'label' => '' , 'class' => $status_class.' js-checkbox-list'));?>
						</td>
						<td class="text-center">
							<div class="dropdown">
								...
								<ul class="dropdown-menu">
									<li>
										<?php echo $this->Html->link('<i class="fa fa-pencil-square-o fa-fw"></i>'.__l('Edit'), array( 'action'=>'edit', $repaymentSchedule['RepaymentSchedule']['id']), array('class' => '','escape'=>false, 'title' => __l('Edit')));?>
									</li>
									<li>
										<?php echo $this->Html->link('<i class="fa fa-times fa-fw"></i>'.__l('Delete'), Router::url(array('action'=>'delete',$repaymentSchedule['RepaymentSchedule']['id']),true).'?r='.$this->request->url, array('class' => 'js-confirm ', 'escape'=>false,'title' => __l('Delete')));?>
									</li>
									<?php echo $this->Layout->adminRowActions($repaymentSchedule['RepaymentSchedule']['id']);  ?>
								</ul>
							</div>
						</td>
						<td class="text-center"><?php echo $this->Html->cText($repaymentSchedule['RepaymentSchedule']['name']);?></td>
						<td class="text-center"><?php echo $this->Html->cInt($repaymentSchedule['RepaymentSchedule']['day']);?></td>
						<td class="text-center"><?php echo $this->Html->cBool($repaymentSchedule['RepaymentSchedule']['is_particular_day_of_month']);?></td>
					</tr>
					<?php
					endforeach;
					else:
					?>
					<tr>
						<td colspan="7" class="text-center text-danger"><i class="fa fa-exclamation-triangle fa-fw"></i> <?php echo sprintf(__l('No %s available'), __l('repayment schedules'));?></td>
					</tr>
					<?php
					endif;
					?>
